 ...
 *
 * Keys the push solvers' tables (every state width).  The values come from
 * splitmix64 (independent, uniformly random words), so a key -- an XOR of such
 * words, i.e. simple tabulation hashing -- is already a good hash: the tables
 * take their index from the key's low bits directly (SOK_IDX), with no mixing
 * step per probe.  (They used to come from xorshift64, a GF(2)-linear
 * generator, whose XOR relations made raw key bits a poor index.)
 * ======================================================================== */
#define USE_ZOBRIST
 ...
 * invent a shortcut and prune a valid level).  Generated by splitmix64 from
 * another seed, independent of the primary set. */
...
static void zobrist_init(void) {
    /* splitmix64 streams with fixed seeds (reproducible); the primary set is
     * drawn for every cell (not only this grid's), like the second set */
    uint64_t s = 0x9E3779B97F4A7C15ULL;
    for (int c = 0; c < MAX_NCELLS;  c++) z_player[c] = splitmix64_next(&s);
    for (int m = 0; m < 16;          m++)
        for (int c = 0; c <= MAX_NCELLS; c++) z_block[m][c] = splitmix64_next(&s);
    for (int h = 0; h < MAX_NCELLS;  h++) z_hole[h] = splitmix64_next(&s);
...
/* Table tiers.  A solve starts on tier 0, the SMALL table (64K slots, 1 MB,
 * L2-resident), which serves the overwhelming majority of shortcut checks
 * (~100 states).  When a solve's inserts reach half of its tier, the table
 * GROWS IN PLACE to the next tier, 4x larger (64K, 256K, 1M, 4M, then HTP_SIZE
 * = 16M slots): the entries are re-inserted in their original insertion order
 * (the touched list), so the new tier holds exactly what inserting them there
 * from the start would have given, and every pending queue entry is pointed at
 * its new slot.  The solve goes on where it was: no work is repeated, and the
 * result, the pop order and the pop count do not depend on the tier (keys are
 * compared in full; below the last tier the load stays <= 1/2, so probing
 * never gives up).  (It used to restart from scratch on the 256 MB table once
 * the small one overflowed: on heavy 6x6 jobs that restart cost ~25% of all
 * pops, and every such solve ran on a table far too big for the TLB.)  The
...
#if HTP_SMALL_SIZE >= HTP_SIZE || (HTP_SIZE & (HTP_SIZE - 1))
#error "HTP_SIZE must be a power of two larger than the small table (1 << HTP_SMALL_LG2)"
#endif
#ifndef HTP_KEEP_LG2
#define HTP_KEEP_LG2 22        /* tiers of >= 4M slots (64 MB) are freed after use */
#endif
#define HTP_NTIERS_MAX 16

/* Index of key k in a table of mask+1 slots: the key's low bits (the Zobrist
 * words are uniformly random, see ZOBRIST HASHING). */
#define SOK_IDX(k) ((uint64_t)(k))

typedef struct {
    HSlot64 *slot;               /* active tier's table                       */
    uint32_t ht_mask;            /* active tier size - 1                      */
    uint32_t ht_limit;           /* insert count at which the tier is full    */
    uint32_t ht_count;           /* inserts this solve                        */
    int      overflow;           /* set when ht_count reached ht_limit        */
    int      tier;               /* active tier (0 = small)                   */
    HSlot64 *tiers[HTP_NTIERS_MAX];   /* tier tables, allocated on first use (tier 0 at start) */
    int      big_used;           /* the last solve used a tier of >= HTP_KEEP_LG2: free it when the next one starts */
    uint32_t ht_seq;
    ...
    void    *ms_tiers[HTP_NTIERS_MAX];   /* multi-start label tables (MSLab), by tier (lazy) */
    size_t   ms_bytes[HTP_NTIERS_MAX];   /* their sizes, for the generation-wrap clear in hsp64_clear */
    int      ms_tier;            /* active label tier */
    uint32_t ms_mask, ms_limit;  /* its size - 1 and insert limit */
    uint32_t *touched;           /* slots inserted this solve, in insertion order (every tier) */
    uint32_t  ntouched, touched_cap;
    int       last_exhaustive;   /* last cutoff solve ran to completion with no win */
...
static int g_htp_ntiers;         /* number of tiers: HTP_SMALL_LG2, +2, +2, ..., log2(HTP_SIZE) */
static long long g_tier_grows[HTP_NTIERS_MAX], g_ms_grows[HTP_NTIERS_MAX];   /* growths into tier t (push tables, label tables) */
void sokoban_table_stats(long long *push_grows, long long *ms_grows, int n) {
    for (int t = 0; t < n && t < HTP_NTIERS_MAX; t++) { push_grows[t] = g_tier_grows[t]; ms_grows[t] = g_ms_grows[t]; }
}

static int htp_tier_lg(int t) {
    int big = __builtin_ctzll((unsigned long long)(HTP_SIZE));
    int lg = HTP_SMALL_LG2 + 2 * t;
    return lg < big ? lg : big;
}
...
static HashStatePush64 *hsp64_get(void) {
    if (!hsp64_tls) {
        ...
        hsp64_tls->ht_seq = 1;
        hsp64_tls->tiers[0] = sok_xcalloc(HTP_SMALL_SIZE, sizeof(HSlot64), "small push table");
        hsp64_tls->touched_cap = HTP_SMALL_SIZE / 2;
        hsp64_tls->touched = sok_xmalloc(hsp64_tls->touched_cap * sizeof(uint32_t), "push table index");
        g_htp_ntiers = 1;
        while (htp_tier_lg(g_htp_ntiers - 1) < __builtin_ctzll((unsigned long long)(HTP_SIZE))) g_htp_ntiers++;
    }
    return hsp64_tls;
}

/* Make tier t the active table (allocating it on first use). */
static void hsp64_set_tier(HashStatePush64 *hs, int t) {
    const int lg = htp_tier_lg(t);
    const uint32_t size = 1u << lg;
    if (!hs->tiers[t]) hs->tiers[t] = sok_xcalloc(size, sizeof(HSlot64), t == g_htp_ntiers - 1 ? "big push table" : "push table tier");
    hs->tier = t; hs->slot = hs->tiers[t]; hs->ht_mask = size - 1;
    hs->ht_limit = (t == g_htp_ntiers - 1) ? (uint32_t)(size * 0.85) : size / 2;
    if (hs->touched_cap < hs->ht_limit) {
        hs->touched = sok_xrealloc(hs->touched, (size_t)hs->ht_limit * sizeof(uint32_t), "push table index");
        hs->touched_cap = hs->ht_limit;
    }
    if (lg >= HTP_KEEP_LG2) hs->big_used = 1;
}

/* Start a solve on the small table.  A tier of >= HTP_KEEP_LG2 slots used by
 * the previous solve (whose labels may have been exported since) is freed. */
static void hsp64_select_small(HashStatePush64 *hs) {
    if (hs->big_used) {
        for (int t = 1; t < g_htp_ntiers; t++)
            if (hs->tiers[t] && htp_tier_lg(t) >= HTP_KEEP_LG2) { free(hs->tiers[t]); hs->tiers[t] = NULL; }
        /* the insertion index grew with them (4 B per insert): back to the small
         * tier's size (its contents are dead between solves; tiers regrow it) */
        free(hs->touched);
        hs->touched_cap = HTP_SMALL_SIZE / 2;
        hs->touched = sok_xmalloc(hs->touched_cap * sizeof(uint32_t), "push table index");
        hs->big_used = 0;
    }
    hs->tier = 0; hs->slot = hs->tiers[0]; hs->ht_mask = HTP_SMALL_SIZE - 1;
...
static void hsp64_clear(HashStatePush64 *hs) {
    if (++hs->ht_seq == 0) {
        ...
        for (int t = 0; t < g_htp_ntiers; t++)
            if (hs->tiers[t]) memset(hs->tiers[t], 0, ((size_t)1 << htp_tier_lg(t)) * sizeof(HSlot64));
        for (int t = 0; t < HTP_NTIERS_MAX; t++) if (hs->ms_tiers[t]) memset(hs->ms_tiers[t], 0, hs->ms_bytes[t]);
        hs->ht_seq = 1;
    }
    hs->pq_count = 0;
    hs->probe_failed = 0;
 ...
 * active tier has reached its insert limit; the caller then grows the table
 * (or, on the last tier, ends the solve with SOK_TABLE_FULL) and repeats the
 * update. */
static inline int hsp64_update(HashStatePush64 *hs, uint64_t k, int new_cost) {
    const uint32_t mask = hs->ht_mask;
    uint64_t h = SOK_IDX(k) & mask;
    for (int i = 0; i < HT_PROBE_LIMIT; i++) {
...
/* Move this solve's entries from the active tier to the next one (see the
 * TABLE TIERS comment).  Afterwards the old tier's slot i holds, in its cost
 * field, the index its entry now has (the old tier is not read again by this
 * solve), which is how the caller re-points its pending queue entries. */
static HSlot64 *hsp64_grow_rehash(HashStatePush64 *hs) {
    HSlot64 *old = hs->slot;
    hsp64_set_tier(hs, hs->tier + 1);
    g_tier_grows[hs->tier]++;
    HSlot64 *nw = hs->slot;
    const uint32_t mask = hs->ht_mask, seq = hs->ht_seq;
    for (uint32_t j = 0; j < hs->ntouched; j++) {
        HSlot64 *so = &old[hs->touched[j]];
        uint32_t h = (uint32_t)(SOK_IDX(so->key) & mask), p = 0;
        while (nw[h].gen == seq) { h = (h + 1) & mask; p++; }   /* keys are distinct: the first free slot, as an insert would take */
        if (p >= HT_PROBE_LIMIT) hs->probe_failed = 1;          /* an insert here would have given up: the result is then unproven */
        nw[h] = *so;
        so->cost = (int32_t)h;
        hs->touched[j] = h;
    }
...
/* The near-goal frontier profile (BfsProfile.tail_width) is filled only while
 * this is on (the default; the worker turns it off unless a beam option reads
 * it, which saves a per-pop update and zeroing 256 bytes per check). */
static int g_tail_prof = 1;
void sokoban_set_tail_profile(int on) { g_tail_prof = on ? 1 : 0; }
...
static inline void bfs_tail_reset(BfsProfile *prof, int max_cost) {
    ...
    if (g_tail_prof) for (int i = 0; i < BFS_TAIL_W; i++) prof->tail_width[i] = 0;
}
/* ========================================================================
...
int sokoban_ref_build(const uint64_t *keys, const int32_t *costs, int n, SokRefTable *t) {
    ...
    /* (Re)allocate when too small, and also when a big buffer would serve a far
     * smaller table: a pooled table's capacity must not stay at its high-water
     * mark (a parent table of a heavy check can take tens of MB). */
    if (t->cap < sz || (t->cap > 4 * sz && t->cap > 65536)) { free(t->hk); free(t->hc); t->hk = sok_xmalloc(sz * sizeof(uint64_t), "reference table keys"); t->hc = sok_xmalloc(sz * sizeof(int32_t), "reference table costs"); t->cap = sz; }
    memset(t->hk, 0, sz * sizeof(uint64_t));
    t->mask = sz - 1; t->n = 0;
    g_refstat[4]++; g_refstat[5] += n;
    for (int i = 0; i < n; i++) {
        uint64_t key = keys[i] ? keys[i] : 1;
        uint32_t h = (uint32_t)(SOK_IDX(key) & t->mask);
...
static inline int ref_lookup(uint64_t key) {
    key ^= g_ref_xor;
    if (!key) key = 1;
    uint32_t h = (uint32_t)(SOK_IDX(key) & REF_MASK);
...
/* Export the labels of the last exhaustive cutoff solve (on whichever tier it
 * ended) as a table of upper bounds on the solved state's forward distances;
 * -1 if there are max entries or more.  Every label in the
...
int sokoban_export_settled(uint64_t *keys, int32_t *costs, int max) {
    ...
    if (!hs->last_exhaustive || hs->ntouched >= (uint32_t)max) return -1;
    int n = 0;
    for (uint32_t i = 0; i < hs->ntouched && n < max; i++) {
        HSlot64 *sl = &hs->slot[hs->touched[i]];
...
#ifndef MS_MAX_LG2
#define MS_MAX_LG2 (MS_LG2 + 2)             /* the label table grows like the push tables, 4x per tier, up to 2^MS_MAX_LG2 slots */
#endif
#if MS_MAX_LG2 < MS_LG2 || MS_MAX_LG2 > 26
#error "MS_MAX_LG2 must be MS_LG2..26"
#endif
#ifndef MS_ARENA
...
/* Label-table tiers: 2^MS_LG2, 4x larger per tier, up to 2^MS_MAX_LG2 slots;
 * a tier takes inserts up to half its size, and a multi solve that fills the
 * last one ends with -2 (the caller's exact per-start fallback).  Growth moves
 * the labels like the push tables' (see TABLE TIERS): same insertion order,
 * pending entries and cached successors re-pointed, no work repeated. */
static int ms_tier_lg(int t) { int lg = MS_LG2 + 2 * t; return lg < MS_MAX_LG2 ? lg : MS_MAX_LG2; }
static int ms_ntiers(void) { int n = 1; while (ms_tier_lg(n - 1) < MS_MAX_LG2) n++; return n; }
static MSLab *ms_set_tier(HashStatePush64 *hs, int t) {
    const uint32_t size = 1u << ms_tier_lg(t);
    if (!hs->ms_tiers[t]) { hs->ms_bytes[t] = (size_t)size * sizeof(MSLab); hs->ms_tiers[t] = sok_xcalloc(size, sizeof(MSLab), "multi-start labels"); }
    hs->ms_tier = t; hs->ms_mask = size - 1; hs->ms_limit = size / 2;
    if (hs->touched_cap < hs->ms_limit) {
        hs->touched = sok_xrealloc(hs->touched, (size_t)hs->ms_limit * sizeof(uint32_t), "push table index");
        hs->touched_cap = hs->ms_limit;
    }
    return (MSLab *)hs->ms_tiers[t];
}

/* The label slot of the state with keys (k, k2).  -1: table full (overflow
 * set: the caller grows the table or, on the last tier, gives up), probe
 * limit, or a primary-key collision (same k, different k2): the caller then
 * ends the multi solve with -2 and the starts are decided one by one, which is
 * exact. */
static inline int ms_slot(HashStatePush64 *hs, MSLab *L, uint64_t k, uint64_t k2, int *is_new) {
    const uint64_t kt = MS_KEY(k);   /* the full key is stored (a pop reads it back); matches and the index use MS_KEY */
    const uint32_t mask = hs->ms_mask;
    uint64_t h = SOK_IDX(kt) & mask;
    for (int i = 0; i < HT_PROBE_LIMIT; i++) {
        uint32_t idx = (uint32_t)((h + i) & mask);
        MSLab *sl = &L[idx];
        if (sl->gen != hs->ht_seq) {
            if (hs->ht_count >= hs->ms_limit) { hs->overflow = 1; return -1; }
...
const char *sokoban_knobs_json(void) {
    ...
    snprintf(buf, sizeof buf,
             "\"HTP_SIZE\":%lld,\"HTP_SMALL_LG2\":%d,\"HTP_KEEP_LG2\":%d,\"HP64_SIZE\":%lld,\"HT_PROBE_LIMIT\":%lld,"
             "\"REF_LG2\":%d,\"MS_LG2\":%d,\"MS_MAX_LG2\":%d,\"MS_ARENA\":%lld,\"SOK_W1_BITS\":%d,\"SOK_W2_BLOCKS\":%d,"
             "\"SOK_STATE_BITS_MAX\":%d,\"DIRECT_LIMIT\":%lld,\"HT_SIZE\":%lld,\"QSZ\":%lld,\"solver_defs\":\"%s\"",
             (long long)(HTP_SIZE), (int)(HTP_SMALL_LG2), (int)(HTP_KEEP_LG2), (long long)(HP64_SIZE), (long long)(HT_PROBE_LIMIT),
             (int)(REF_LG2), (int)(MS_LG2), (int)(MS_MAX_LG2), (long long)(MS_ARENA), (int)(SOK_W1_BITS), (int)(SOK_W2_BLOCKS),
...
/* A pending queue entry.  Its state's key is not stored: a pop reads it from
 * the table slot (hs->slot[slot].key in the push solvers, the label slot in
 * the multi-start solver), which the stale-entry check touches anyway.  No
 * per-entry used-direction bits (see fill_used_dirs). */
typedef struct {
    SOKN(PSt) state;       /* packed blocks + hole mask (for unpacking) */
    uint32_t  slot;        /* table slot holding this state           */
    int32_t   prio;        /* bucket priority: g, or f = g + h in the cutoff solver */
    int32_t   g;           /* exact cost so far (walks + pushes)      */
    int8_t    player_pos;  /* exact player cell (not in the packed state) */
} SOKN(HeapE);             /* w1 24 B, w2 36 B, w4 52 B */
...
/* The active tier is full: move the solve to the next tier (see TABLE TIERS)
 * and re-point every pending entry at its state's new slot. */
static void SOKN(grow)(HashStatePush64 *hs, BucketV *bq) {
    const HSlot64 *old = hsp64_grow_rehash(hs);
    for (int b = 0; b < PQ64_NBUCKETS; b++) {
        SOKN(HeapE) *it = (SOKN(HeapE) *)bq[b].items;
...
static int SOKN(solve_push)(const Puzzle *pz, uint8_t *used_dirs, BfsProfile *prof) {
    ...
    {
        SOKN(HeapE) e0;
        e0.prio = 0; e0.g = 0; e0.player_pos = (int8_t)pz->player_start;
        SOKN(pst_pack)(&e0.state, ib, nb, ihm);
        e0.slot = (uint32_t)hsp64_update(hs, zpack64(pz->player_start, ib, pz->block_pushable, nb, ihm, nh, pz->hole_pos), 0);
    ...
    while (hs->pq_count > 0) {
        ...
        const uint64_t ekey = hs->slot[e.slot].key;
        n_popped++;
        ...
        for (int bi = 0; bi < nb; bi++) {
            ...
            for (int d = 0; d < 4; d++) {
                ...
                uint64_t nk = ekey ^ z_player[e.player_pos] ^ z_player[bpos]
                            ^ z_block[mb][bpos] ^ z_block[mb][new_bpos];
                if (ch >= 0) nk ^= z_hole[pz->hole_pos[ch]];
                __builtin_prefetch(&hs->slot[SOK_IDX(nk) & hs->ht_mask], 1, 1);
                cand[ncand].key  = nk;
        ...
        for (int ci = 0; ci < ncand; ci++) {
            ...
            if (slot >= 0) {
                ...
                SOKN(HeapE) ne;
                ne.prio = nc; ne.g = nc; ne.player_pos = (int8_t)cand[ci].bpos;
                ...
                /* The tier is full: grow the table (see TABLE TIERS) and repeat
                 * this candidate.  The last tier full: undecided (-5). */
                if (hs->tier + 1 >= g_htp_ntiers) {
                    if (prof) { prof->peak_heap_sz = peak_heap; prof->states_popped = n_popped; }
...
static int SOKN(solve_push_cutoff)(const Puzzle *pz, uint8_t *used_dirs, BfsProfile *prof, int max_cost) {
    ...
    HashStatePush64 *hs = hsp64_get();
    BucketV *bq = hs->bq[SOKW_IX];
    const int tail = prof && g_tail_prof;
    hsp64_select_small(hs);
    hsp64_clear(hs);
    SOKN(bq_reset)(bq);
#ifdef FWPROF
    ...
    {
        SOKN(HeapE) e0;
        e0.g = 0; e0.player_pos = (int8_t)pz->player_start;
#ifndef NO_ASTAR
    ...
    while (hs->pq_count > 0) {
        ...
        for (int ci = 0; ci < ncand; ci++) {
            ...
            if (slot >= 0) {
                ...
                SOKN(HeapE) ne;
                ne.prio = nprio; ne.g = nc; ne.player_pos = (int8_t)cand[ci].bpos;
                ne.slot = (uint32_t)slot;
                SOKN(pst_move)(&ne.state, &e.state, nb, cand[ci].bi, cand[ci].nbp, cand[ci].ch >= 0 ? 1u << cand[ci].ch : 0u);
                SOKN(bq_push)(hs, bq, &ne);
                if (hs->pq_count > peak_heap) peak_heap = hs->pq_count;
...
/* The label table is full: move this solve's labels to the next tier (see the
 * label-table tiers above) and re-point the pending entries and the cached
 * successors.  Returns the new table; afterwards the old table's soff field of
 * each moved slot holds that slot's new index (for the caller's own indices). */
static MSLab *SOKN(ms_grow)(HashStatePush64 *hs, BucketV *bq, SOKN(MSSucc) *AR, uint32_t arena_n, MSLab **old_out) {
    MSLab *old = (MSLab *)hs->ms_tiers[hs->ms_tier];
    MSLab *nw = ms_set_tier(hs, hs->ms_tier + 1);
    g_ms_grows[hs->ms_tier]++;
    const uint32_t mask = hs->ms_mask, seq = hs->ht_seq;
    for (uint32_t j = 0; j < hs->ntouched; j++) {
        MSLab *so = &old[hs->touched[j]];
        uint32_t h = (uint32_t)(SOK_IDX(MS_KEY(so->key)) & mask), p = 0;
        while (nw[h].gen == seq) { h = (h + 1) & mask; p++; }
        if (p >= HT_PROBE_LIMIT) hs->probe_failed = 1;   /* ms_slot would have given up there: the solve ends with -2 */
        nw[h] = *so;
        so->soff = h;
        hs->touched[j] = h;
    }
    for (int b = 0; b < PQ64_NBUCKETS; b++) {
        SOKN(HeapE) *it = (SOKN(HeapE) *)bq[b].items;
        for (int i = 0; i < bq[b].len; i++) it[i].slot = old[it[i].slot].soff;
    }
    for (uint32_t a = 0; a < arena_n; a++) if (AR[a].slot >= 0) AR[a].slot = (int32_t)old[AR[a].slot].soff;
    hs->overflow = 0;
    *old_out = old;
    return nw;
}
...
static int SOKN(solve_multi)(const Puzzle *pz, const int8_t *starts, const int16_t *cut, const uint32_t *pred,
    ...
    SOKN(MSSucc) *AR = SOKN(ms_arena_tls); uint32_t arena_n = 0;
    BucketV *bq = hs->bq[SOKW_IX];
    hsp64_select_small(hs);   /* (this solve keys the label table L; the push tiers stay unused) */
    hsp64_clear(hs);   /* a generation wrap clears L too (see hsp64_clear) */
    MSLab *L = ms_set_tier(hs, 0);
    const int ms_last = ms_ntiers() - 1;
    SOKN(bq_reset)(bq);
    mand_setup(pz);
...
#define MS_DECIDE_SHORT(i0) do { uint32_t _q = 1u << (i0); \
    ...
    for (int bi = 0; bi < nb; bi++) {
        ...
        for (int d = 0; d < 4; d++) {
            ...
            for (uint32_t t = alive; t; t &= t - 1) {
                ...
                if (slot < 0) {
                    slot = ms_slot(hs, L, nk, nk2, &is_new);
                    if (slot < 0 && hs->overflow && hs->ms_tier < ms_last) { MSLab *old; L = SOKN(ms_grow)(hs, bq, AR, arena_n, &old); slot = ms_slot(hs, L, nk, nk2, &is_new); }
                    if (slot < 0) return -2;
                }
#ifndef MS_NO_DOM
                ...
                SOKN(HeapE) e; e.prio = nc; e.g = nc; e.player_pos = (int8_t)bpos; e.slot = (uint32_t)slot; e.state = ns;
    ...
    while (hs->pq_count > 0 && alive) {
        ...
        uint32_t eslot = e.slot;     /* (re-pointed if the table grows while this state is replayed) */
        MSLab *lb = &L[eslot];
        uint32_t R = 0;
        ...
        if (!lb->cached) {
            ...
            for (int bi = 0; bi < nb; bi++) {
                ...
                for (int d = 0; d < 4; d++) {
                    ...
                    if (mand_dead_ctx(pz, &mc, bp, nb, bi, new_bpos, (uint32_t)nhm, ch)) continue;
                    uint64_t nk = lb->key ^ z_player[e.player_pos] ^ z_player[bpos] ^ z_block[mb][bpos] ^ z_block[mb][new_bpos];
        ...
        for (int k = 0; k < lb->scount; k++) {
            ...
            int slot = sc->slot, is_new = 0, improved = 0;   /* probe once, reuse on every replay (a growth re-points sc->slot) */
            ...
            for (uint32_t t = R; t; t &= t - 1) {
                ...
                if (slot < 0) {
                    slot = ms_slot(hs, L, sc->key, sc->key2, &is_new);
                    if (slot < 0 && hs->overflow && hs->ms_tier < ms_last) {
                        MSLab *old; L = SOKN(ms_grow)(hs, bq, AR, arena_n, &old);
                        eslot = old[eslot].soff; lb = &L[eslot];
                        slot = ms_slot(hs, L, sc->key, sc->key2, &is_new);
                    }
                    if (slot < 0) return -2;
                    sc->slot = slot;
            ...
            SOKN(HeapE) ne; ne.prio = nc; ne.g = nc; ne.player_pos = sc->bpos; ne.slot = (uint32_t)slot; ne.state = sc->state;
            SOKN(bq_push)(hs, bq, &ne);
            if (hs->pq_count > peak) peak = hs->pq_count;
        }
    }
